    ...
    def findHand(self, img, draw=True):
        ...
        if self.result.multi_hand_landmarks:  # Check if any hand landmarks are detected
            for handLms in self.result.multi_hand_landmarks:
                if draw:
                    landmark_col = self.mpDraw.DrawingSpec(color=(255, 0, 0), thickness=1) #BGR(BLUE, GREEN, RED) BLUE COLOR AND POINTS WITH DOT
                    connection_col = self.mpDraw.DrawingSpec(color=(0, 0, 255), thickness=1) # RED COLOR JUST LINE CONNECTING
    ...
    def findPos(self, img, handNo=0, draw=True):
        ...
        if self.result.multi_hand_landmarks:
            ...
            for id, lm in enumerate(myHand.landmark):
                ...
                # THIS IF CONDITION FOR  SHOW HAND FINGURES WITH PROPER DOTES IN BLUR COLOR
                # if draw:
